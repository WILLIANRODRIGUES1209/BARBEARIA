...
import { format, parseISO, startOfDay, endOfDay, isWithinInterval, startOfMonth, endOfMonth, startOfWeek, endOfWeek, subMonths, subDays } from 'date-fns';
...
export default function AdminMeuHistorico() {
  ...
  const periodLabel = useMemo(() => {
    if (filterMode === 'ALL') return 'Todo o Histórico';
  ...
  }, [state.appointments, state.services, state.transactions, currentBarbeiroId, comissaoPercent, barbeiro, startDate, endDate, filterMode]);
      ...
      <div className="bg-[#0C0C0C] border border-[#222] p-4 sm:p-5 rounded-2xl flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3 bg-[#141414] border border-[#333] p-2.5 rounded-xl">
          <div className="flex items-center gap-1 overflow-x-auto custom-scrollbar pb-1 lg:pb-0 shrink-0">
            ...
            >
              <CalendarIcon size={14} /> Período Personalizado
            </button>
            {filterMode !== 'DEFAULT' && (
              <button
                onClick={() => setFilterMode('DEFAULT')}
